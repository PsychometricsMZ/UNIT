#!/usr/bin/env python3
"""Scenario A pool runner (tau_share=25%).
...
POOL_DIR = REPO_ROOT / 'results' / 'pool_scenarioa'
...
def run_experiment(n_samples: int, seed: int) -> dict:
    ...
    from rpmdeep.dgp.mediation import generate_mediation_data, VAR_SHARES_A
    ...
    try:
        data = generate_mediation_data(
            n_samples, seed, var_shares=VAR_SHARES_A, lambda_k=LAMBDA_K
        )
...
def _experiment_path(pool_dir: Path, n: int, seed: int) -> Path:
    return pool_dir / f'scena_n{n:06d}_s{seed:05d}.json'
...
def _print_summary(df: pd.DataFrame) -> None:
    ...
    print('\n' + '=' * 88)
    print(f'Scenario A pool — tau_share=25%  ({len(df)} experiments)')
    print('=' * 88)
    for n in SAMPLE_SIZES:
...
def _load_existing(pool_dir: Path) -> list:
    records = []
    for p in sorted(pool_dir.glob('scena_n*_s*.json')):
        try:
...
def main():
    ...
    logging.info(f'[Scenario A pool] {POOL_DIR}')
